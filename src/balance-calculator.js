...
export function getRemainingPlayerCount(players) {
  return players.filter((p) => !getPlayerSellStatus(state.currentLeagueId, p.i))
    .length;
}
...
export function updateBalanceBadge() {
  ...
  const { sellValue, projectedBalance } = calculateBalanceData(
...
export function updateTeamValueBadge() {
  ...
  const { totalValue } = calculateBalanceData(
    state.currentPlayers,
    state.currentBudget,
  );
  const teamValueDiff = calculateTeamValueDiff(state.currentPlayers);
  const diffClass =
    teamValueDiff > 0 ? "positive" : teamValueDiff < 0 ? "negative" : "";
...
export function generateBalanceBadgeHTML(
  budget,
  projectedBalance,
  sellValue,
  plannedCost = 0,
  ...
  const projectedBalanceHTML = hasProjected
    ? html`<span class="badge-value projected" id="projected-balance"
        >${formatCurrency(projectedBalance)}</span
      >`
    : html`<span
        class="badge-value projected"
  ...
  return html`
    <div class="stat-badge balance-badge">
      ...
      <div class="balance-stack">
        ...
        <span class="badge-value current" id="balance-value"
...
export function generateTeamValueBadgeHTML(totalValue, teamValueDiff) {
  ...
  const diffHTML =
    teamValueDiff !== 0
      ? html`<span class="badge-value diff ${diffClass}" id="team-value-diff"
          >${teamValueDiff > 0 ? "+" : ""}${formatCurrency(teamValueDiff)}</span
        >`
      : html`<span
          class="badge-value diff"
          id="team-value-diff"
          style="display: none;"
        ></span>`;
  ...
  return html`
    <div class="stat-badge value-badge">
      ...
      <div class="value-stack">
        ${diffHTML}
        <span class="badge-value current" id="team-value"
