    ...
    class extends t {
      ...
      update(s, [t, r, c]) {
        ...
        for (; x <= j && k <= w; )
          ...
          else if (h[x] === a[k]) ((v[k] = o(d[x], p[k])), x++, k++);
          else if (h[j] === a[w]) ((v[w] = o(d[j], p[w])), j--, w--);
          else if (h[x] === a[w])
            ((v[w] = o(d[x], p[w])), i(s, v[w + 1], d[x]), x++, w--);
          else if (h[j] === a[k])
            ((v[k] = o(d[j], p[k])), i(s, d[x], d[j]), j--, k++);
          else if (
            (void 0 === m && ((m = u(a, k, w)), (y = u(h, x, j))), m.has(h[x]))
          )
            if (m.has(h[j])) {
              const e = y.get(a[k]),
                t = void 0 !== e ? d[e] : null;
